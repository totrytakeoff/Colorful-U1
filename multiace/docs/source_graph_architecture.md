...
基础设施，但前端 UI 还没有按新架构重构，通用 source transition 也仍处于
预览和 dry-run 验证阶段，尚未接入正式打印执行路径。
...
- 正式打印 rewrite 仍以 route plan v2 为主，但还没有把同一 head 多 source 的
  unload/load transition 自动插入打印 G-code。
...
状态：已完成 preview 骨架，尚未接入正式打印执行。`POST
/api/source-transition/preview` 能根据当前 head 的 `current_source` 生成
`unload_source -> select_head -> load_source/swap_source` 计划片段。
...
1. 收敛 post-processor：
   ...
   - 增加同一 head 多 source 的 transition rewrite dry-run。
2. 把 `source-transition/preview` 的 planner 复用到正式 preflight route plan：
   - 不直接执行硬件动作；
   - 先让最终 G-code 与 preview 命令完全一致；
