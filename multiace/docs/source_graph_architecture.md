...
```json
{
  ...
  "initial_state": {
    "head:0": {"current_source": "native:0", "confidence": "known"},
    "head:1": {"current_source": null, "confidence": "empty"},
    "head:2": {"current_source": null, "confidence": "empty"},
    "head:3": {"current_source": "ace:0:3", "confidence": "known"}
  },
  "tool_map": {
  ...
  "events": [
    {
      ...
      "source": "native:0",
      "head": "head:0",
      "action": "select | load | unload | swap | preload",
      "commands": ["T0"],
      "requires": {
...
1. 新增 `source_graph.json` schema。
2. Web backend 能读写 source graph。
3. dry-run 能返回 source graph fixture。
4. UI 暂时不大改，只先展示 graph。

等 source graph 稳定后，再开始 Phase C/D，把 preflight 和 ACE slot 任意映射接入。
