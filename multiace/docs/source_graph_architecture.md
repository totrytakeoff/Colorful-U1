...
- 正式打印 rewrite 已能消费 route plan v2 中同一 head 多 source 的
  unload/load/swap transition，并已通过 dry-run；尚未实机验证。
...
同一 head 多 source 打印时漏掉第一段 source 状态。旧 `tool_targets` fallback
仍存在，需要继续收敛。
...
1. 收敛 post-processor：
   - route plan v2 作为主输入；
   - 已开始减少 `tool_targets` / `ace_targets` fallback：route plan 模式下
     缺失正文 tool event 或辅助行 target 会失败；
